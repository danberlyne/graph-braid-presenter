...
from splitting_tools.graph_braid_group import GraphBraidGroup
...
def stringify_factors(splitting, braid_factor_counter, gog_factor_counter, include_presentation = False, presentations = None):
    ...
    for i, factor in enumerate(splitting):
        if type(factor) == GraphBraidGroup:
            if factor.is_reduced():
                ...
                if (essential_adj_matrix_hashable, factor.num_particles) in known_gbgs and not include_presentation: 
                    ...
                    if include_presentation:
                        previous_particles = [int(line[11:].strip()) for line in splitting_data if line.startswith('particles')]
                        previous_gbgs = [(previous_matrices[j], previous_particles[j]) for j in range(len(previous_matrices))]
                        is_new_gbg = True
                        for previous_gbg in previous_gbgs:
                            if is_same(essential_graph.adj_matrix, previous_gbg[0]) and factor.num_particles == previous_gbg[1]:
                                splitting[i] = f'B_{previous_gbg[1]}(Gamma_{previous_matrices.index(previous_gbg[0]) + 1})'
                                is_new_gbg = False
                                break
                    is_new_graph = True
                ...
                if include_presentation:
                    ...
                    is_new_gbg = True
                    for previous_gbg in previous_gbgs:
                        if is_same(factor.adj_matrix, previous_gbg[0]) and factor.num_particles == previous_gbg[1]:
                            splitting[i] = f'RB_{previous_gbg[1]}(Gamma_{previous_matrices.index(previous_gbg[0]) + 1})'
                            is_new_gbg = False
                            break
                is_new_graph = True
                for previous_matrix in previous_matrices:
    ...
    for i, factor in enumerate(stringified_splitting):
        ...
        if type(factor) == list:
            ...
            if is_direct:
                ...
                free_rank = improved_sum(int(subfactor[2:]) for subfactor in factor if subfactor.startswith('F_')) + len([subfactor for subfactor in factor if subfactor == 'Z'])
