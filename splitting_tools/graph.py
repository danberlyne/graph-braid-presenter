...
ConnectednessException = Exception('Graph must be connected.')
...
class Graph:
    ...
    def iterate_tree(self, tree, current_vertex):
        for edge in [e for e in self.edges if current_vertex in e]:
            candidate_vertex = edge[edge.index(current_vertex) - 1]
            if candidate_vertex not in tree[0]:
                tree[0].append(candidate_vertex)
                tree[1].append(edge)
                if set(tree[0]) != set(self.vertices):
                    self.iterate_tree(tree, candidate_vertex)